...
class ComplaintManagementApp:
    ...
    def handle_user_menu(self):
        ...
        while True:
            ...
            if choice == "1":
                ...
                complaint_id = int(
                    self.user_view.get_user_input("Enter Complaint ID to delete")
                )
                self.complaint_controller.delete_complaint(
                    complaint_id, self.current_user["id"]
                )
            elif choice == "5":
                self.user_controller.change_password(self.current_user["id"])
            elif choice == "6":
                self.complaint_controller.search_complaints_by_category(
                    self.current_user["id"]
                )
            elif choice == "7":
                self.complaint_controller.export_complaints_to_csv(
                    self.current_user["id"]
                )
            elif choice == "8":
                break
    ...
    def handle_admin_menu(self):
        ...
        while True:
            ...
            if choice == "1":
                ...
                complaint_id = int(
                    self.user_view.get_user_input("Enter Complaint ID to assign")
                )
    ...
    def handle_staff_menu(self):
        ...
        while True:
            ...
            if choice == "1":
                self.complaint_controller.view_assigned_complaints(
                    self.current_user["id"]
                )
            elif choice == "2":
                complaint_id = int(self.user_view.get_user_input("Enter Complaint ID"))
