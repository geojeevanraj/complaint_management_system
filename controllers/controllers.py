...
class UserController:
    ...
    def login(self):
        ...
        try:
            ...
            if user:
                self.user_view.display_success(
    ...
    def change_password(self, user_id: int):
        ...
        try:
            ...
            if user and self.user_service.authenticate_user(
                user["email"], old_password
            ):
...
class ComplaintController:
    ...
    def register_complaint(self, user_id: int):
        ...
        try:
            ...
            if not all([complaint_data["category"], complaint_data["description"]]):
                self.complaint_view.display_error(
                    "Category and description are required"
                )
                return False

            if self.complaint_service.create_complaint(
                user_id, complaint_data["category"], complaint_data["description"]
            ):
    ...
    def assign_complaint(self, complaint_id: int):
        ...
        try:
            staff_email = self.complaint_view.get_user_input(
                "Enter staff email to assign"
            )
    ...
    def delete_complaint(
        ...
        try:
            if self.complaint_service.delete_complaint(complaint_id):
                ...
                self.complaint_view.display_error(
                    "Complaint not found or you do not have permission"
                )
                return False
        except Exception as e:
    ...
    def search_complaints_by_category(
        ...
        try:
            ...
            if is_admin:
                complaints = self.complaint_service.find_complaints_by_category(
                    category
                )
            else:
                complaints = (
                    self.complaint_service.find_complaints_by_user_and_category(
                        user_id, category
                    )
                )
    ...
    def export_complaints_to_csv(
        ...
        user_id: int = None,
        is_admin: bool = False,
        filename: str = "complaints_export.csv",
    ):
    ...
    def add_comment_to_complaint(self, staff_id: int, complaint_id: int):
        ...
        try:
            ...
            if self.comment_service.create_comment(complaint_id, staff_id, comment):
                ...
                self.complaint_view.display_error(
                    "Failed to add comment or complaint not assigned to you"
                )
                return False
        except Exception as e:
