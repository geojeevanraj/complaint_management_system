...
def view_user_complaints(user_id):
    ...
    for c in complaints:
        print(
            f"ID: {c['_id']}, Category: {c['category']}, Status: {c['status']}, Description: {c['description']}"
        )
...
def view_all_complaints():
    ...
    for c in complaints:
        ...
        print(
            f"ID: {c['_id']}, User: {user['name']}, Category: {c['category']}, Status: {c['status']}"
        )
...
def assign_complaint(complaint_id, staff_id):
    complaints_collection.update_one(
        {"_id": ObjectId(complaint_id)}, {"$set": {"assigned_to": ObjectId(staff_id)}}
    )
...
def view_complaints_by_status(status):
    ...
    for c in complaints:
        user = users_collection.find_one({"_id": c["user_id"]})
        print(
            f"ID: {c['_id']}, User: {user['name']}, Category: {c['category']}, Description: {c['description']}"
        )


def export_complaints_to_csv(
    user_id=None, is_admin=False, filename="complaints_export.csv"
):
    query = {}
    if user_id and not is_admin:
        query["user_id"] = ObjectId(user_id)
...
def view_complaint_statistics():
    ...
    print(
        f"Total: {total}, Pending: {pending}, In Progress: {in_progress}, Resolved: {resolved}"
    )
                ...
                if user["role"] == "user":
                    ...
                    while True:
                        ...
                        if a_choice == "1":
                            ...
                            status = input(
                                "Enter status to filter (Pending/In Progress/Resolved): "
                            ).strip()
